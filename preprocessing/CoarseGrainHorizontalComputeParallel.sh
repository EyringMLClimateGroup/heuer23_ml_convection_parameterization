...
#SBATCH --nodes=1
#SBATCH --ntasks=37            # Specify max. number of tasks to be invoked
...
for SLURM_ARRAY_TASK_ID in {0..36}; do
    ...
    start_hcg="srun --ntasks=1 --cpus-per-task=4 --mem=8000 cdo remap,${target_grid_path},${weight_file} -sellonlatbox,$box_m2degr -setgrid,${source_grid_path} $in_path ${out_path}"
    #start_hcg="srun --ntasks=1 --cpus-per-task=4 --mem=8000 cdo remapcon,${target_grid_path} -sellonlatbox,$box_m2degr -setgrid,${source_grid_path} $in_path ${out_path}"
